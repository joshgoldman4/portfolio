...
void test_translate_num_2() {
    long int output;

    CU_ASSERT_EQUAL(translate_num(&output, "1234", -10, 2000), 0);
    CU_ASSERT_EQUAL(output, 1234);
    CU_ASSERT_EQUAL(translate_num(&output, "145", -140, 145), 0);
    CU_ASSERT_EQUAL(output, 145);
    CU_ASSERT_EQUAL(translate_num(&output, "0xBEEF", -50000, 50000), 0);
    CU_ASSERT_EQUAL(output, 48879);
    CU_ASSERT_EQUAL(translate_num(&output, "BEEF", -50000, 50000), -1);
    CU_ASSERT_EQUAL(translate_num(&output, "-17", -16, 71), -1);
    CU_ASSERT_EQUAL(translate_num(&output, "+72", 72, 150), 0);
    CU_ASSERT_EQUAL(output, 72);
    CU_ASSERT_EQUAL(translate_num(&output, "72", 150, 0), -1);
    CU_ASSERT_EQUAL(translate_num(&output, "0xJFK", -100, 100), -1);
    CU_ASSERT_EQUAL(translate_num(&output, "123$!", 0, 700), -1);
    CU_ASSERT_EQUAL(translate_num(&output, "&43", -100, 100), -1);
}
...
void test_table_3() {
    ...
    retval = add_to_table(tbl, "first", 4);
    CU_ASSERT_EQUAL(retval, 0);
    retval = add_to_table(tbl, "sec", 32);
    CU_ASSERT_EQUAL(retval, 0);
    retval = add_to_table(tbl, "third", 28);
    CU_ASSERT_EQUAL(retval, 0);
    retval = add_to_table(tbl, "sec", 24); 
    CU_ASSERT_EQUAL(retval, -1); 
    retval = add_to_table(tbl, "bob", 10); 
    CU_ASSERT_EQUAL(retval, -1); 
    retval = add_to_table(tbl, NULL, 24); 
    CU_ASSERT_EQUAL(retval, -1); 
    retval = add_to_table(NULL, "bob", 12); 
    CU_ASSERT_EQUAL(retval, -1); 

    retval = get_addr_for_symbol(tbl, "abc");
    CU_ASSERT_EQUAL(retval, -1); 
    retval = get_addr_for_symbol(tbl, "first");
    CU_ASSERT_EQUAL(retval, 4); 
    retval = get_addr_for_symbol(tbl, "third");
    CU_ASSERT_EQUAL(retval, 28); 
    retval = get_addr_for_symbol(tbl, "sec");
    CU_ASSERT_EQUAL(retval, 32); 
    retval = get_addr_for_symbol(tbl, "bob");
    CU_ASSERT_EQUAL(retval, -1);
    ...
    for (int i = 0; i < 12; i += 1) {
    	retval = add_to_table(tbl2, "same", 40);
    	CU_ASSERT_EQUAL(retval, 0);
    }
    retval = add_to_table(tbl2, "same", 88); 
    CU_ASSERT_EQUAL(retval, 0);
    retval = add_to_table(NULL, "same", 88); 
    CU_ASSERT_EQUAL(retval, -1);
    retval = add_to_table(tbl2, NULL, 88); 
    CU_ASSERT_EQUAL(retval, -1);
    retval = add_to_table(tbl2, "same", 87); 
    CU_ASSERT_EQUAL(retval, -1);
...
int main(int argc, char** argv) {
    ...
    if (!CU_add_test(pSuite1, "test_translate_num_2", test_translate_num_2)) {
        goto exit;
    }
